...
RUN /opt/shibboleth-idp/bin/plugin.sh --noPrompt -i \
        https://shibboleth.net/downloads/identity-provider/plugins/oidc-common/2.0.0/oidc-common-dist-2.0.0.tar.gz \
    && /opt/shibboleth-idp/bin/plugin.sh --noPrompt -i \
        https://shibboleth.net/downloads/identity-provider/plugins/duo-oidc/1.2.0/idp-plugin-duo-nimbus-dist-1.2.0.tar.gz \
